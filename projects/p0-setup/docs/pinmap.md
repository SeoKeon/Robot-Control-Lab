...
- **J8 두 핀이 I2C1 이 된다 — 확인 완료** (CubeMX 칩 DB 의 AF 목록):
  ...
  **PB8 은 BOOT0 겸용 핀**이지만 이 보드는 옵션 바이트가 nSWBOOT0=0 /
  nBOOT0=1 로 설정돼 있어(2026-08-29 실측) **핀 상태와 무관하게 항상 플래시로
  부팅**한다. I2C 풀업으로 SCL 이 리셋 때 High 여도 문제없다. 단, 칩 교체나
  옵션 바이트 초기화 후에는 이 설정이 풀릴 수 있다 — "엔코더 꽂으면 부팅이 안
  된다" 증상이 나오면 여기를 의심할 것:
  `STM32_Programmer_CLI -c port=SWD -ob displ` 로 확인.
...
- **AS5600 실제 배선** (보드가 3.3V 전용 — 구매페이지·솔더점퍼로 확인):
  VCC→**J4 패드 3번(3.3V)**, GND→J8 GND, SDA→J8 B+/H2(PB7), SCL→J8 Z+/H3(PB8),
  DIR→GND, OUT·GPO 미연결. J4 핀아웃(1=SWDIO, 2=SWCLK, 3=3V3, 4=GND)은
  UM2516 Table 6 + 회로도 교차 확인. ⚠️ J4 3번 납땜 시 옆의 SWDIO/SWCLK 패드와
  브리지 나면 디버깅이 끊긴다
